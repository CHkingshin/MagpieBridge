...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    @PostConstruct//当前类初始化后执行
    private void init() {
        //类初始化后直接执行异步任务 对队列中的订单进行读取
        SECKILL_ORDER_EXECUTOR.submit(new VoucherOrderHandler());
    }
    private IVoucherOrderService proxy;
    /**
     * @description  阻塞队列异步优化下单流程
     * @date 2022/8/15
     */
    @Override
    public Result seckillVoucher(Long voucherId) {
        ...
        voucherOrder.setUserId(orderId);//用户Id
        voucherOrder.setVoucherId(orderId);//代金券id
    ...
    public void createVoucherOrder(VoucherOrder voucherOrder) {
        // 5.一人一单
        Long userId = voucherOrder.getUserId();//子线程不能从threadLocal里获得id
        // 5.1.查询订单
    ...
    /**
     * @description
     * 读取队列中的订单信息的异步线程任务 在类初始化之后就开始进行该线程的动作
     * @author KingShin
     * @date 2022/8/15 20:52:27
     */
    ...
    private void handleVoucherOrder(VoucherOrder voucherOrder) {
        ...
        boolean isLock = lock.tryLock();//拿锁
        //判断是否拿锁成功
        if(!isLock){
            log.error("不允许重复下单！");
            return;
